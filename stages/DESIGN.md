...
三份 PNG 從同版 SVG 內嵌的原圖解碼輸出，沿用原檔名；減少動態模式只顯示這張原圖。README 提供靜態圖與文件站入口；文件站提供停止／播放控制，無 JavaScript 或系統偏好減少動態時使用 PNG，PDF 組稿也使用 PNG。GitHub 是否播放必須以 README 嵌入實測，不能把檔案預覽等同 README；不支援時保留完整靜態呈現，不轉 GIF，也不提高圖片容量上限。
