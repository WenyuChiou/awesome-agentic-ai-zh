...
Banner 由 `scripts/build-banner.py` 在原版插圖上加動畫，不重畫文字、圖示、卡片、底部四格或接線。三語沿用各自原圖與既有位置，共用路線順序及 18 秒時間軸：共用基礎 2 秒、Track A 6 秒、Track B 8 秒、全圖停留 2 秒；只有路線光點與節點外框加亮。SVG 內嵌高品質壓縮的原圖，不使用 JavaScript、外部圖片或外部字型。7.5 的進階選讀與角色依需求選讀寫在替代說明，不為動畫重排原圖。

三份 PNG 從同版 SVG 內嵌的原圖解碼輸出，沿用原檔名；減少動態模式只顯示這張原圖。README 提供靜態圖與文件站入口；文件站提供停止／播放控制，無 JavaScript 或系統偏好減少動態時使用 PNG，PDF 組稿也使用 PNG。GitHub 是否播放必須以 README 嵌入實測，不能把檔案預覽等同 README；不支援時保留完整靜態呈現，不轉 GIF。這是頂部 banner 的單一例外，不改變其餘概念圖的 Image 2.0 PNG 規則，也不提高圖片容量上限。
