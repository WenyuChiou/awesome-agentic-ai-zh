...
三份 PNG 從同版 SVG 內嵌的原圖解碼輸出，沿用原檔名；減少動態模式只顯示這張原圖。README 只保留文件站入口，避免在首屏放兩個視覺相同的 banner 入口；PNG 仍保留給文件站的停止／播放控制、無 JavaScript 或系統偏好減少動態時的 fallback，以及 PDF 組稿。GitHub 是否播放必須以 README 嵌入實測，不能把檔案預覽等同 README；不支援時 SVG 本身仍須呈現完整靜態內容，不轉 GIF，也不提高圖片容量上限。
