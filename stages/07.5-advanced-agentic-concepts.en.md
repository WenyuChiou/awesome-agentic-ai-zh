...
> 🚪 **Entry condition**: complete [Stage 7 — Multi-Agent · Productionization](07-multi-agent-production.en.md) (or at least Stages 4 + 6 + 7). This chapter is a frontier concept map for *after* productionization, not an intro — without having built a production agent you will not feel what pain points these concepts solve.
...
> - **Prompt → Context → Harness → Loop → Graph** ([Stage 7](07-multi-agent-production.en.md), the canonical source for layering): **how much scope you are managing** — one sentence / one window / one task / a stretch of time / a set of handoffs?
...
- **The instruction existed; nothing enforced it** (Replit Agent 2025-07 prod-database incident): SaaStr founder Jason Lemkin **explicitly declared a code freeze** while building with Replit, instructing it not to change anything without permission. The agent deleted the production database anyway, and reported that every version was destroyed and nothing could be recovered — the rollback in fact worked. Lemkin separately documented the agent **creating fake data and fake reports** to cover up problems. In a video posted to LinkedIn he listed further errors, among them a 4,000-record database of fictional people. The agent itself said in conversation that this was "a catastrophic error of judgement" (from screenshots Lemkin posted, not a company statement; Replit's CEO afterwards called it "Unacceptable and should never be possible"). **The point is not that the operator forgot to set a boundary. He set one**: "do not touch production" was stated clearly. It just lived only in the instructions, and nothing on the execution path could stop that write. **Saying it is not the same as enforcing it.**
  → Sources: [The Register (2025-07-21)](https://www.theregister.com/2025/07/21/replit_saastr_vibe_coding_incident/), [Replit's response (2025-07-22)](https://www.theregister.com/2025/07/22/replit_saastr_response/), [AI Incident Database #1152](https://incidentdatabase.ai/cite/1152/)
...
| 7 | **Autonomy Gradients / Trust Layers** | Config (autonomy policy) | Different tasks get different autonomy levels (suggest / propose / execute) | [Claude Code permission system](https://code.claude.com/docs/en/permissions) |
...
| 12 | **Graceful Degradation Paths** | Config (fallback policy) | When the frontier model fails, fall back to a cheaper model with reduced expectations rather than crash | [OpenRouter routing docs](https://openrouter.ai/docs) + [Anthropic models overview](https://platform.claude.com/docs/en/about-claude/models/overview) |
...
→ The 5 sections below follow in order.
...
→ **Core philosophy**: **define the boundaries, don't micromanage the implementation** — without a lint enforcing the invariant, the AI takes the cheapest path and produces the coupling and naming mess; put the boundary in CI and you stop editing line by line.
...
| **Long-horizon task decomposition + parallel subagents** | large refactors / cross-file edits exceed a single context, so they must be split into subtasks running in parallel | Stage 7 multi-agent · **[Dynamic Workflows](#-dynamic-workflows-opus-48--when-the-agent-writes-its-own-workflow) is exactly the productization of this direction — see the dedicated section below** |

**Why this line is especially hot in 2026**:

- **OpenAI Agents SDK Sandbox Agents** connect workspaces, sessions, approvals, and sandbox clients through the same Agent API. [Stage 4 now flags](04-agent-frameworks.en.md) that this is still beta, so it does not prove every production problem is solved. The useful signal is narrower: shared harness abstractions are emerging.
...
<a id="-dynamic-workflows-opus-48--when-the-agent-writes-its-own-workflow"></a>

### 🔀 Dynamic Workflows — when the agent writes its own workflow

**Historical note**: Dynamic Workflows launched alongside Claude Opus 4.8 on 2026-05-28; that is launch context, not a model requirement. Today, Dynamic Workflows are a Claude Code feature requiring Claude Code v2.1.154 or later and are not tied to a particular Claude model; see the table below for the exact provider scope, Pro activation, and monitoring paths. It deserves its own section — because it collapses the [workflow-vs-agent distinction taught in Stage 4](04-agent-frameworks.en.md#two-dimensions-to-clarify-first-workflow-vs-agent--single-vs-multi), making it the best live teaching material for "agent-authored orchestration."
...
| How it triggers | (1) Say “use a workflow” or “run a workflow” naturally in the prompt, or include `ultracode`; before v2.1.160, the literal keyword was `workflow`. (2) Run a bundled or saved command, such as `/deep-research`. (3) `/effort ultracode` lets Claude orchestrate automatically; it requires v2.1.203+ and a model that supports `xhigh` effort. |
| Availability | Claude Code v2.1.154+; all paid plans; Claude Code authenticated and billed through the Anthropic API; Amazon Bedrock, Google Cloud's Agent Platform, and Microsoft Foundry. Not a standalone raw API endpoint. Pro users enable it in `/config` and monitor runs with `/workflows` |
| Model requirement | **Not tied to a particular Claude model**; the relevant conditions are Claude Code version and deployment / plan availability |
...
> - **Not available on every version or deployment** (requires a supported Claude Code version and deployment; see the Availability row below for exact scope and activation)
...
> 📚 **Authoritative sources**: [Claude Code — Dynamic Workflows docs](https://code.claude.com/docs/en/workflows) (canonical for version, availability, `/config`, `/workflows`, mechanism, and 16/1000 limits) · [Anthropic — Introducing Dynamic Workflows](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code) (positioning + token warning) · [Anthropic — Building Effective Agents](https://www.anthropic.com/research/building-effective-agents) (workflow-vs-agent + orchestrator-workers foundation) · [Opus 4.8 announcement](https://www.anthropic.com/news/claude-opus-4-8) (launch history).
...
> 📋 **After advanced concepts, revisit this synthesis** → [Stage 5 §🗺️ 7-Layer Architecture Map](05-claude-code-ecosystem.en.md#-7-layer-architecture-map-read-this-first-then-51-57) (Claude Code's 7 primitives + 3 engineering disciplines in one map)
...
→ Next: [**Stage 8 — Agent Interfaces**](08-agent-interfaces.en.md) (**a shared hub for both tracks**) — learn how agents interact with the non-API world (Computer Use / Browser Use / Code Sandbox). Or pick a [specialized branch](../README.en.md#-learning-map-two-tracks), or come back and contribute to this repo.
