...
| **長 horizon 任務分解 + parallel subagent** | 大型重構 / 跨檔修改超過單一 context、要拆成 subtask 平行跑 | Stage 7 multi-agent · **[Dynamic Workflows](#-dynamic-workflowsopus-48-當-agent-自己寫出-workflow) 就是這個方向的 product 化——見下方專節** |
...
### 🔀 Dynamic Workflows — 當 agent 自己寫出 workflow

**歷史註記**：Dynamic Workflows 在 2026-05-28 與 Claude Opus 4.8 一起發布；這是發布背景，不代表功能綁定某個 Claude model。現在，Dynamic Workflows 是 Claude Code 功能，需要 Claude Code v2.1.154+，不綁定特定 Claude model；完整的 provider 可用範圍、Pro 啟用方式與監看方式見下表。它值得單獨拆開——因為它把 [Stage 4 教的 workflow vs agent 區分](04-agent-frameworks.md#兩個維度先分清楚workflow-vs-agent--single-vs-multi)整個塌縮掉，是理解「agent-authored orchestration」最好的活教材。
...
| 怎麼觸發 | (1) 直接在 prompt 說「use a workflow」或「run a workflow」，或加 `ultracode`；v2.1.160 前 literal keyword 才是 `workflow`。(2) 用 bundled/saved command，例如 `/deep-research`。(3) `/effort ultracode` 讓 Claude 自動編排；需 v2.1.203+，且 model 支援 `xhigh` effort。 |
| 可用範圍 | Claude Code v2.1.154+；所有付費方案；由 Anthropic API 驗證 / 計費的 Claude Code；Amazon Bedrock、Google Cloud Agent Platform、Microsoft Foundry。不是獨立的 raw API endpoint。Pro 在 `/config` 啟用，並以 `/workflows` 監看執行進度 |
| Model 要求 | **不綁定特定 Claude model**；關鍵條件是 Claude Code 版本與部署 / 方案可用性 |
...
> - **不是任意版本或 deployment 都能用**（需要支援的 Claude Code 版本與 deployment；詳細可用範圍與啟用方式見下方「可用範圍」列）
...
> 📚 **權威來源**：[Claude Code — Dynamic Workflows docs](https://code.claude.com/docs/en/workflows)（版本、可用範圍、`/config`、`/workflows`、機制 + 16/1000 上限的 canonical）· [Anthropic — Introducing Dynamic Workflows](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code)（定位 + token 警告）· [Anthropic — Building Effective Agents](https://www.anthropic.com/research/building-effective-agents)（workflow vs agent + orchestrator-workers 根基）· [Opus 4.8 announcement](https://www.anthropic.com/news/claude-opus-4-8)（發布歷史）。
