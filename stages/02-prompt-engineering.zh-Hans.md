...
| 先给一两个好答案 | 先看例题 | 范例；few-shot prompting |
...
用同一段客服留言，分别运行“帮我整理”和四部分版本。写下一项看得见的差别。

<details markdown="1">
<summary>展开范例、Path A／B 和完成条件</summary>
...
不要把“请写出完整思维链（chain-of-thought）”当成通用解法。新一代推理模型通常会在内部完成推理；需要核对时，要求**最后答案后附一段简短、可验证的理由**即可。
