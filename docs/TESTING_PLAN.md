...
`scripts/check-reader-ux.py` ratchets each locale's visible mainline with a narrow measured ceiling, requires at least six visible required-reading links plus 21 visible resource links and ratings, and locks all 18 core-term definitions before Core Exercise 1. `scripts/check-image-locale.py` ensures the English and Simplified Chinese pages use their own bright image variants. The localized relationship diagrams show Harness as the work environment, Loop as the repeat-and-check rhythm, Graph as the branching route, and Eval as a ruler across the system without implying a product-generation sequence. The localized Eval Case diagrams show input as one field inside a complete case rather than the whole Golden／Reference Set.
