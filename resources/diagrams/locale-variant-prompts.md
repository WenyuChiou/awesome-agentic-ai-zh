...
- 可編輯來源：`scripts/build-banner.py` 的路線與節點座標；SVG 自包含原圖的 WebP quality 95 資料，payload SHA-256 固定在產生器。這是高品質有損壓縮，不宣稱與原 PNG 位元相同；沒有縮放、裁切、改字或重畫。首次以 `--import-original --png` 從固定 commit 匯入；平常用 `python scripts/build-banner.py --png` 重製覆蓋層與同名靜態 PNG，`--check` 比對來源與成品。
