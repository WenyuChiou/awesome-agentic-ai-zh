...
- **頂部 Banner 試版例外**：明確核准的 README banner 使用自包含動畫 SVG，內嵌各語言的原版插圖，不重畫構圖、字體、圖示或配色。三語共用路線順序與 18 秒時間軸，光點依各自原圖接線移動、節點外框短暫加亮。內嵌原圖使用高品質壓縮，同版 PNG 提供靜態、減少動態與 PDF 使用；文件站有停止／播放控制，README 有靜態圖入口。SVG 不含 JavaScript 或外部資源，不轉 GIF，不提高容量上限；其他教學圖仍依上面的 PNG 規則。
